"""Run paired comparison: uncalibrated vs calibrated (Platt scaling) logistic baseline.

Orchestrates:
  1. Two backtests (w6_logistic_uncalibrated, w6_logistic_calibrated)
  2. Per-run evaluation + report generation
  3. MLflow logging (if available)
  4. Named comparison: uncalibrated_vs_calibrated
  5. Summary table + MANIFEST.json

Usage:
    uv run python scripts/run_calibration_comparison.py
"""
...
CONFIGS = [
    ("uncalibrated", "configs/w6_logistic_uncalibrated.yaml"),
    ("calibrated", "configs/w6_logistic_calibrated.yaml"),
]
...
def main() -> None:
    ...
    # ── 3. Comparison ──
    comp_name = "uncalibrated_vs_calibrated"
    pair_dirs = [Path(results[l]["run_dir"]) for l in ("uncalibrated", "calibrated")]
    ...
    for label, info in results.items():
        ...
        cal_m = ev.get("calibration_metrics", {})
        summary_rows.append({
            "variant": label,
            "run_id": info["run_id"],
            ...
            "ece": cal_m.get("ece", "N/A"),
            "brier": cal_m.get("brier", "N/A"),
        })
